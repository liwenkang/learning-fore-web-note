...
* 但是存在很多问题，例如
    ...
    * 样式不能跨页面复用，跟上一条类似
        - 虽然使用一些服务端的技术可以达到标签属性的样式复用，但是流量依然会有所浪费（p12）
    ...
    - 什么是CSS？
        ...
        + 层叠 样式表
            * 层叠在这里是什么意思呢？（p12）
                * 冲突处理
                    * 不是代码合并冲突
                    * 而是说对同一个元素指定了不同的样式，哪一个生效(不存在同一属性,在同一元素上同时生效的情况,比如给文字加上删除线和下划线)
                * 样式是可以继承的
...
* 元素（即html标签）的层级关系
    * 嵌套/层叠/树形/递归/自相似/分形结构
        ...
        - 祖先 ancestor
    * 分类标准一: 替换元素 / 非替换元素
        * <p>你在哪</p>，h123445,div,span,a,i,strong,em,select input textarea    
        * 可以填充内容,所以是非替换元素
        * 下面的标签不能填充内容,也就是说它的内容实际上是 src 中的内容
        * <img src="a.jpg" />
        * <video src="a.mp4">
            <source>
          </video>
        * <audio src="a.mp3">
            wliejfoiwjef(此处的文字无效)
        </audio>
